...
目前專案已經改為單一 Raspberry Pi 主體架構，所以：

- 不再保留 `apps/pc-server`
- 不再保留舊的 `apps/` 包裝層
- 主程式碼直接放在 `src/robot_core/`
- Python 專案設定放在 repo root 的 `pyproject.toml`
...
```bash
./scripts/start_robot_core.sh --goal "find toy"
```
